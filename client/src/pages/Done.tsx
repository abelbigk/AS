...
const DARK_IMAGE = "https://images.unsplash.com/photo-1470770903676-69b98201ea1c?w=1600&q=80";
const LIGHT_IMAGE = "https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=1600&q=80";

export default function Done() {
  const [, navigate] = useLocation();
  const { data: items, isLoading: itemsLoading } = trpc.content.listByStatus.useQuery("done");
  ...
  return (
    <div className="relative min-h-screen page-enter">
      ...
      <div className="fixed inset-0 -z-10">
        {/* Dark mode background — aurora reflections */}
        ...
        {/* Light mode background — warm golden morning field */}
        <img src={LIGHT_IMAGE} alt="" className="block dark:hidden w-full h-full object-cover" />
        <div className="block dark:hidden absolute inset-0 backdrop-blur-xl bg-white/40" />
      </div>
      ...
      <div className="container mx-auto px-4 py-8 md:py-12 max-w-2xl">
        ...
        {groupedData.length > 0 ? (
            ...
            {groupedData.map((group, index) => (
              <div key={group.category.id} style={{ animationDelay: `${index * 50}ms` }} className="stagger-item space-y-3">
                <CategoryCard 
                  ...
                  onClick={() => navigate(`/category/${group.category.id}?status=done`)} 
                />
          ...
          <div className="flex flex-col items-center justify-center py-20">
            <p className="text-gray-600 dark:text-white/60 text-lg">Nothing completed yet</p>
            <p className="text-gray-400 dark:text-white/40 text-sm mt-2">Mark items as finished to see them grouped here</p>
          </div>
        )}
      </div>
      )}
