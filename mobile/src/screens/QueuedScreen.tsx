...
export default function QueuedScreen({ navigation }: TabProps<"Queued">) {
  const { theme } = useTheme();
  const itemsQuery = trpc.content.listByStatus.useQuery("queued");
  ...
  return (
    <SafeAreaView style={[styles.flex, { backgroundColor: theme.background }]} edges={["top"]}>
      <View style={styles.headerPad}>
        <Text style={[styles.title, { color: theme.text }]}>Queued</Text>
        <Text style={{ color: theme.textMuted, marginBottom: 16 }}>
          {itemsQuery.data?.length ?? 0} items waiting
        </Text>
      </View>
        ...
        renderItem={({ item }) => (
          <View style={styles.rowWrap}>
            <CategoryCard
              ...
              onPress={() =>
                navigation.navigate("CategoryDetail", { categoryId: item.category.id, status: "queued" })
              }
            />
          </View>
        )}
        ListEmptyComponent={<EmptyState title="No queued items" subtitle="Queue content from any card menu." />}
