...
export default function DoneScreen({ navigation }: any) {
  const { theme } = useTheme();
  const isDark = theme === 'dark';
  const { data: items = [], isLoading, refetch } = trpc.content.listByStatus.useQuery('done');
  ...
  const renderGroupHeader = ({ section }: any) => (
    ...
    >
      <View style={styles.groupTitleSection}>
        ...
        <Text style={styles.itemCount}>{section.count} completed</Text>
      </View>
        ...
        renderItem={({ item }) => (
          <View style={styles.itemCard}>
            <View style={styles.itemContent}>
              <Text style={[styles.itemTitle, styles.completedTitle]}>{item.heading}</Text>
